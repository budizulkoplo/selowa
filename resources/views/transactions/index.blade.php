    ...
    <div class="ibox">
        ...
        <div class="ibox-content">
            ...
            <form method="POST" action="{{ route('transactions.store') }}" class="row m-b" id="transactionForm">@csrf
                <div class="col-sm-4"><select name="customer_id" id="transactionCustomer" class="form-control select2" required><option value="">Pilih pelanggan</option>@foreach($customers as $customer)<option value="{{ $customer->id }}" data-price="{{ $customer->customer_price }}">{{ $customer->name }} - Rp {{ number_format($customer->customer_price, 0, ',', '.') }}</option>@endforeach</select></div>
                ...
                <div class="col-sm-2"><button class="btn btn-primary btn-block"><i class="fa fa-plus"></i> Tambah</button></div>
            ...
            <div class="table-responsive">
                <table class="table table-striped">
                    <thead><tr><th>Tanggal</th><th>Kode</th><th>Pelanggan</th><th>Qty</th><th>Harga</th><th>Total</th><th class="text-right">Aksi</th></tr></thead>
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="7" class="text-center text-muted">Belum ada transaksi pada bulan ini.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
